    ...
    <div className="min-h-screen bg-gray-50 pb-28 font-sans text-right" dir="rtl">
      ...
      <div className="bg-white px-4 pt-10 pb-4 sticky top-0 z-20 shadow-sm rounded-b-3xl">
        <div className="flex gap-2 mb-4">
          ...
          <div className="relative flex-1">
            <input
              ...
              className="w-full bg-gray-50 border border-gray-100 rounded-2xl py-3 pr-12 pl-4 text-sm font-bold text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none transition-colors"
            />
            <Search size={20} className="absolute right-4 top-3.5 text-gray-400" />
          </div>
        </div>

        {/* 2. زر التبديل (خريطة / قائمة) */}
        <div className="flex bg-gray-100 rounded-2xl p-1 relative">
          ...
          <button
            onClick={() => setView('map')}
            className={`flex-1 flex justify-center items-center gap-2 py-3 text-sm font-black z-10 transition-colors ${
      ...
      {/* 3. محتوى الصفحة (الخريطة أو القائمة) */}
      <div className="h-[calc(100vh-220px)] w-full">
        {view === 'map' ? (
          // عرض الخريطة (صورة وهمية مع نقاط مؤقتة حتى نربطها بخرائط جوجل الحقيقية لاحقاً)
          <div className="relative w-full h-full bg-emerald-50 overflow-hidden">
            ...
            {/* دوائر المطاعم (Pins) مثل تطبيق TGTG */}
            ...
            <div className="absolute bottom-[30%] left-[20%] bg-emerald-800 text-white w-14 h-14 rounded-full flex items-center justify-center font-black border-[3px] border-white shadow-xl text-lg">12</div>
            <div className="absolute top-[20%] left-[30%] bg-emerald-800 text-white w-10 h-10 rounded-full flex items-center justify-center font-black border-[3px] border-white shadow-xl">1</div>
            
            {/* نقطة موقع المستخدم الحالي (زرقاء) */}
          ...
          // عرض القائمة (نفس البطاقات في الصفحة الرئيسية)
          <div className="p-6">
            <p className="text-center font-bold text-gray-500 mt-10">سيتم عرض قائمة العروض هنا بنفس شكل البطاقات الجميلة في الرئيسية 🚀</p>
          </div>
        )}
      </div>
