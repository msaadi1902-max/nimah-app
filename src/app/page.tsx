...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // 🕵️‍♂️ تحديث تلقائي (Realtime): بمجرد إضافة وجبة في قاعدة البيانات تظهر هنا فوراً
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28 text-right font-sans" dir="rtl">
      ...
      <div className="p-4 space-y-6 mt-2">
        ...
        {loading ? (
          <div className="text-center font-bold text-emerald-700 py-10 animate-pulse">جاري البحث عن العروض... 🍕</div>
        ) : meals.length === 0 ? (
          <div className="text-center bg-white p-8 rounded-3xl border border-gray-100 shadow-sm mt-4">
            <p className="text-gray-500 font-bold mb-4">لا توجد عروض حالياً</p>
          </div>
        ) : (
          meals.map((meal) => (
            <Link href={`/offer/${meal.id}`} key={meal.id} className="block bg-white rounded-[20px] shadow-sm border border-gray-200 overflow-hidden relative cursor-pointer hover:shadow-md transition-shadow mb-6">
              <div className="relative h-44 w-full bg-emerald-50">
                ...
                {meal.quantity > 0 && (
                  <div className="absolute top-3 left-3 bg-amber-100 px-2 py-1 rounded-lg shadow-sm font-black text-xs text-amber-800">باقي {meal.quantity}</div>
                )}
              </div>
              ...
              <div className="p-4 pt-4">
                <div className="flex justify-between items-start mb-1">
                  <div> 
                    ...
                    <p className="text-gray-500 text-sm font-bold">عرض خاص</p>
                  </div>
                  <button className="text-emerald-700 bg-emerald-50 p-2 rounded-full"><Heart size={20} /></button>
                </div>
                <div className="mt-4 pt-3 border-t border-gray-100 border-dashed flex justify-between items-center text-xs font-bold text-gray-500">
                   استلام: {meal.pickup_time}
                </div>
